...
public class AbstractDao<T> {
	...
	public List<T> findAll(Class<T> clazz, boolean existIsActive) {
		String entityName = clazz.getSimpleName(); //lay ten cua entity
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT o FROM ").append(entityName).append(" o");
		if (existIsActive == true) { //kiem tra isActive = 1
	...
	public T findOne(Class<T> clazz, String sql, Object... params) {
		...
		List<T> result = query.getResultList();//tranh truong hop null neu getSingle thi loi khi null
		if (result.isEmpty()) {
			return null;
		}
		return result.get(0);
	...
	public List<T> callStored(String namedStored, Map<String, Object> params) {
		...
		params.forEach((key, value) -> query.setParameter(key, value));// cach duyet map key value day la cach viet cua java8 
